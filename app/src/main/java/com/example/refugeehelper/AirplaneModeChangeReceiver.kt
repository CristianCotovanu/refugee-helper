...
class AirplaneModeChangeReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (isAirplaneModeEnabled) {
            ...
            Toast.makeText(context, "Airplane Mode Enabled. You will not see any data in the application anymore.", Toast.LENGTH_LONG).show()
        } else {
            // showing the toast message if airplane mode is disabled
            Toast.makeText(context, "Airplane Mode Disabled. Application Data will be back soon", Toast.LENGTH_LONG).show()
        }
    }
}